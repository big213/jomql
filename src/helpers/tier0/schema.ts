    ...
    for (const prop in schema.resolvers[type]) {
      ...
      if (schema.resolvers[type][prop].args) {
        for (const arg in schema.resolvers[type][prop].args) {
          argsArray.push({
            key: arg,
            ...schema.resolvers[type][prop].args[arg],
          });
        }
      }
        ...
        argsArray
          .map(
            (ele) =>
              ele.key +
              ": " +
              sanitizeType(ele.type) +
              (ele.required ? "!" : "")
          )
          .join(", ") +
        ")";
      output.type[capitalizeString(type)][keyname] =
        sanitizeType(schema.resolvers[type][prop].type) +
        (schema.resolvers[type][prop].allowNull ? "" : "!");
    }
  }
      ...
      if (!schema.typeDefs[type][prop].hidden) {
        ...
        if (schema.typeDefs[type][prop].args) {
          for (const arg in schema.typeDefs[type][prop].args) {
            argsArray.push({
              key: arg,
              ...schema.typeDefs[type][prop].args[arg],
            });
          }
        }
        ...
        const keyname =
          ...
          (argsArray.length
            ? "(" +
              ...
              ")"
            : "");
        output.type[capitalizedType][keyname] =
          sanitizeType(schema.typeDefs[type][prop].type) +
          (schema.typeDefs[type][prop].allowNull ? "" : "!");
      }
    }
  }
